...
when that column is present in the tuning grid. Each candidate forest is
summarized by a surrogate greedy CI tree, and the grid is ranked by held-out CI
validation gain from that surrogate. Use `parallel_over = "tuning"` to
parallelize grid/fold tasks, or `parallel_over = "forest"` to grow trees within
each forest in parallel after setting a `future` plan.
