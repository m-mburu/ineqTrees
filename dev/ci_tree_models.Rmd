...
ci_splitfun <- function(rank_name,
  ...
  function(model, trafo, data, subset, weights, whichvar, ctrl) {
    ...
    weights <- .ci_validate_weights(weights, nrow(mf))
...
.fit_ci_tree_from_frame <- function(mf,
  ...
  if (length(tree$variable.importance)) {
    fit$variable.importance <- tree$variable.importance
  }
  fit
}
```
...
.ci_validate_weights <- function(weights, n, arg = "weights") {
  ...
  if (any(is.na(weights) & !is.nan(weights))) {
    stop(sprintf("`%s` must not contain missing values.", arg), call. = FALSE)
  }
  if (!all(is.finite(weights))) {
    stop(sprintf("`%s` must contain only finite values.", arg), call. = FALSE)
  }
  if (any(weights < 0)) {
    stop(sprintf("`%s` must be non-negative.", arg), call. = FALSE)
  }
  if (!any(weights > 0)) {
    stop(sprintf("at least one `%s` value must be positive.", arg),
      call. = FALSE
    )
  }
