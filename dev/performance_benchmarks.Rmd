---
title: "Performance Benchmarks for Greedy CI Trees"
output: github_document
---

# Purpose

This development notebook establishes baseline timings and profiling evidence
before changing the greedy concentration-index tree internals. Keep these
diagnostics simple and repeatable. The goal is not to produce
publication-quality performance results, but to identify which functions are
actually expensive before optimizing them.

The first benchmark and profiling targets are:

- `rank_wt()` and `ci_factory()` because every CI score depends on them.
- `weighted_ci_gain()` because it is called repeatedly inside split search.
- `ctree_ci()` on a representative tree fit.
- `cf_ci()` on a small greedy forest.
- `best_numeric_split()` on a large numeric predictor.
- `best_factor_split()` on a high-cardinality factor.

Run this file manually during development. It is not part of the package API and
does not need to be inflated by `fusen`.

For a non-interactive run, use:

```r
source("dev/run_performance_benchmarks.R")
```
...
This uses the package `kenya` example data when available. To keep the benchmark
quick enough for repeated development runs, sample a fixed-size complete subset.
...
These timings establish the baseline before implementing the performance plan.

```{r baseline-benchmarks, eval=FALSE}
...
# Rprof Call-Stack Profiles

`Rprof()` and `summaryRprof()` are built into base R. These profiles give a
reproducible text call stack, which is useful for package development because
the output can be saved in `dev/profiles/` and compared after each optimization.

The profile set below uses a medium-sized subset. This is deliberate: the data
need to be large enough to expose repeated work, but small enough that profiling
can be run often during development.

```{r rprof-profiles, eval=FALSE}
...
# Memory Profiles with `lobstr`

`lobstr` is optional and listed in `Suggests`. These checks are useful for
seeing which calls allocate larger result objects or leave the session with
higher memory use. They are not a substitute for `Rprof()`, but they help
separate CPU-heavy functions from allocation-heavy functions.

```{r lobstr-memory, eval=FALSE}
...
Save the baseline locally so later changes can be compared with the same
settings.

```{r save-results, eval=FALSE}
utils::write.csv(
  baseline_results,
  file = here::here("dev", "performance_baseline_results.csv"),
  row.names = FALSE
)
```
...
1. Run the setup and data chunks.
2. Run the `Rprof()` and optional `lobstr` chunks to identify expensive calls.
3. Run the baseline benchmark chunk before performance changes.
4. Save the results to `dev/performance_baseline_results.csv`.
5. Implement one optimization at a time.
6. Re-run the same profiles and benchmarks.

For each optimization, also run the relevant correctness tests. A faster split
search is only acceptable if it preserves the selected split, gain, terminal
node assignments, and predictions where deterministic behavior is expected.
